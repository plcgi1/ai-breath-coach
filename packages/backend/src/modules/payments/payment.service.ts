...
export class PaymentService {
  ...
  async createInvoiceLink(pricingId: string, userId: string) {
    const pricing = await this.pricingModel.findByPk(pricingId, {
      include: [Policy],
    });
    ...
    try {
      const response = await axios.post(`${this.botApiUrl}/createInvoiceLink`, {
        ...
        prices: [
          { label: pricing.name, amount: Math.round(Number(pricing.price)) },
        ],
      });
  ...
  async handleSuccessfulPayment(payload: {
    ...
    return await this.userSubscriptionsModel.create({
