<script>
    ...
    let data = { techniques: [], user: { purchased_slugs: [] } };
    ...
    function handlePaymentSuccess(slug) {
        ...
        data.user.purchased_slugs = [...data.user.purchased_slugs, slug];
        showPaywall = false;
    ...
    function launchConfetti() {
        ...
        const interval = setInterval(function() {
            ...
            // Вылетает из двух точек по бокам
            confetti({ 
                ...defaults, 
                particleCount, 
    ...
    $: scale = $session.phase === i18n('homepage.inhale') 
        ? 1.25 
        ...
        <footer>
            <div class="slots-grid">
                {#each data.techniques as tech}
                    <button 
                        class="slot {$selectedTech.slug === tech.slug ? 'active' : ''}" 
                        on:click={() => { if (!$session.isRunning) $selectedTech = tech }}
                    >
                        <span class="icon">{tech.icon}</span>
                        {#if !tech.is_free && !data.user.purchased_slugs.includes(tech.slug)}
                            <i class="lock">🔒</i>
                        {/if}
                    </button>
                {/each}
...
<style>
    ...
    .glass-btn:active { background: rgba(255, 255, 255, 0.15); }
    ...
    .stop-btn:active { background: rgba(239, 68, 68, 0.25); }
    ...
    .slots-grid { display: flex; justify-content: center; gap: 18px; }
